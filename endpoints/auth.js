...
module.exports ={
    ...
    removeUnverifiedUsers: async (req, res)=> {
        try{
            const {time} = req.body
            if(!time || time <= 0){
                res.status(200).json({status: true, msg: "Unverified allowed to stay"})
            }
            // Remove unverified Users after 
            filterUsers.removeUnVerifiedUsers(time)
            res.status(200).json({status: true, msg: "Unverified Users removed successfully"})
        }
        catch(err){
            res.status(500).json({status: false, msg: err.message || `Server error contact admin`})
        }
